...
### 인터랙션 상태 (T14, 2026-07-25)

대시보드가 포커스·선택·커서를 갖게 되면서 어느 토큰이 어디에 쓰이는지 못박는다. **토큰 JSON은
변경하지 않는다** — 전부 기존 정의값 소비다.

- **포커스 테두리** = `component.borderFocused`. 포커스된 박스의 테두리에 쓴다(이전에는 Input
  프레임에서만 소비). 색과 **함께** 비색상 단서(제목의 `▸`)를 병행한다.
- **커서** = `component.cursor`(`secondary` 오렌지). 텍스트 입력 커서 전용.
- **선택 행은 배경을 쓰지 않는다.** 라이프사이클 엔트리 선택은 `▌`(borderFocused) + `▸`
  (primary·bold) 거터 마커로 표시한다 — T14 W4의 **B안 채택**. 따라서 `component.selectionBg`를
  대시보드 표면으로 확장하지 않고, T12 배경 정책("배경은 칩만")의 **의도적 칩 예외 목록에 선택 행을
  추가하지 않는다**. 위 "표면" 절의 `#3A2E46`(선택·포커스 표면)은 **Input 프롬프트 한정**으로 읽는다.
- **활성 탭** = `primary` + bold. bold가 색 없이도 같은 신호를 전달한다.
